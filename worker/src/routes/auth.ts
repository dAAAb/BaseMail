...
authRoutes.post('/agent-register', async (c) => {
  ...
  if (!address || !signature || !message) {
    return c.json({
      ...
      hint: 'Step 1: POST /api/auth/start { address } to get the message. Step 2: Sign it with your private key. Step 3: Submit here.',
    }, 400);
  }

  // Verify SIWE signature
  const valid = await verifySiwe(c.env.NONCE_KV, address, signature, message);
  if (!valid) {
    return c.json({ error: 'Invalid signature or expired nonce. Get a fresh message from POST /api/auth/start' }, 401);
  }
  ...
  await c.env.DB.prepare(
    `INSERT INTO accounts (handle, wallet, basename, tx_hash, created_at)
     VALUES (?, ?, ?, NULL, ?)`
  ).bind(handle, wallet, resolved.basename, Math.floor(Date.now() / 1000)).run();
